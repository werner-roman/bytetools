...
describe("KMZ Merger Advanced E2E", () => {
  beforeEach(() => {
    cy.visit("/")
  })

  it("Merges KMZ files in advanced mode without reordering and compares the output - complex files are used", () => {
    ...
    cy.get('input[type="file"]').selectFile([
      'cypress/fixtures/test-data/merger_5.kmz',
      'cypress/fixtures/test-data/merger_7.kmz',
      'cypress/fixtures/test-data/merger_8.kmz',
      'cypress/fixtures/test-data/merger_6.kmz'
    ], {
      force: true
    })
    ...
    // 3. Enable Advanced Mode by clicking the label instead of the switch directly
    cy.get('[data-slot="switch"]').click()
    cy.wait(500) // wait for UI to update after toggle
    ...
    cy.readFile("cypress/downloads/merged_advanced.kmz", null)
      .then(outputFileContent => {
        ...
        return cy.fixture("test-data/merged_advanced_no_Reorder_File_Structure.kmz", null)
